...
- OatFi and Accuray Radixact both use the rich case-study layout now (see
  `frontmatter-schema.md` for the field reference and how to bring a third
  project onto it). Accuray Design System is still the only one on the
  original simpler layout.
- Accuray Radixact is reusing `accuray_main_thumbnail.png` as its `heroImage`
  placeholder and has no images at all yet for its 4 `solutionSections` —
  add these to `public/images/projects/accuray-radiotherapy/` and wire them
  into `accuray-radiotherapy.md`:
  - a dedicated hero screenshot (currently just the home-card thumbnail,
    reused)
  - one image (or several, for the sticky-scroll treatment) per solution
    section: "A shared foundation...", "The image viewer as protagonist",
    "Error handling as a system", "The system underneath"
  - a headshot for Cristina Negrut's testimonial avatar (optional —
    currently unset, falls back to no avatar)
